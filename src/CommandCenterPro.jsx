...
function Section({n,title,icon:Icon,children,wide=false,accent='purple'}){
  return <section className={`proSection ${wide?'wide':''} accent-${accent}`}>
    <header><div className="proSectionIcon"><Icon size={17}/></div><div><span>{String(n).padStart(2,'0')} · COMMAND MODULE</span><h3>{title}</h3></div></header>
    <div className="proSectionBody">{children}</div>
  </section>
}
...
export default function CommandCenterPro(){
  ...
  const [content,setContent]=useState(()=>{
    try{return JSON.parse(localStorage.getItem('memory-space-content-kanban-v1'))||initialContent}
  ...
  useEffect(()=>{
    ...
    syncTarget()
    const id=requestAnimationFrame(syncTarget)
    return()=>cancelAnimationFrame(id)
  },[pathname])
  useEffect(()=>{
    try{localStorage.setItem('memory-space-content-kanban-v1',JSON.stringify(content))}catch{}
  },[content])

  const today=todayKey()
  const tomorrow=tomorrowKey()
  const meetings=useMemo(()=>[...(s.events||[])]
    .filter(x=>x.date===today||x.date===tomorrow)
    .sort((a,b)=>`${a.date}${a.time||''}`.localeCompare(`${b.date}${b.time||''}`)),[s.events,today,tomorrow])
  const note=bookNotes[daySeed()%bookNotes.length]
  ...
  const memoryResults=useMemo(()=>{
    const source=(s.memories||[]).filter(x=>!x.deleted&&!x.archived)
    const q=memoryQuery.trim().toLowerCase()
    if(!q)return source.slice(0,5)
    const words=q.split(/\s+/).filter(Boolean)
    return source.map(m=>({m,score:words.reduce((a,w)=>a+
      (String(m.title||'').toLowerCase().includes(w)?4:0)+
      (String(m.content||'').toLowerCase().includes(w)?2:0)+
      ((m.tags||[]).join(' ').toLowerCase().includes(w)?1:0),0)}))
      .filter(x=>x.score>0).sort((a,b)=>b.score-a.score).slice(0,5).map(x=>x.m)
  },[memoryQuery,s.memories])

  const addTask=()=>{
    if(!quickTask.trim())return
    s.addTodo({task:quickTask.trim(),priority:'medium',due_date:today,category:'Command Center'})
    setQuickTask('')
  }
  const runResearch=type=>{
    if(!researchInput.trim())return
    s.addResearch({
      title:researchInput.trim(),type:type==='business'?'business':'youtube',status:'in-progress',
      summary:type==='business'?'Business idea research queued from Command Center.':'YouTube script generation queued from Command Center.',
      action_items:type==='business'?'Market · competition · offer · economics':'Hook · structure · proof · CTA'
    })
    nav('/research')
  }
  ...
  return createPortal(<div className="proDeck">
    <div className="proDeckHead"><div><span><Sparkles size={13}/> ADVANCED COMMAND CENTER</span><h2>12 intelligent modules, one operating surface.</h2><p>Frontend-ready today. Structured so AI, email, analytics, calendar and finance backends can be connected later.</p></div><button onClick={()=>window.dispatchEvent(new KeyboardEvent('keydown',{key:'k',metaKey:true,bubbles:true}))}>⌘K Universal Search</button></div>
