...
const defaults = {
  ...
  leads:[
    {id:1,name:'Sample Lead',business:'Innogen Prospect',source:'WhatsApp',service:'Company Formation',value:250,status:'new',follow_up:'',notes:'',created_at:nowIso()},
  ],
  clients:[
  ...
  systemMeta:{schemaVersion:2,lastDailyReset:todayKey(),lastBackupAt:null},
  dailyHistory:[],
  activityLog:[],
}

function dailySnapshot(state,date){
  return {
    date,
    routinePct:pct(state.routines),
    focusPct:pct(state.focusBlocks),
    routinesDone:(state.routines||[]).filter(x=>x.done).length,
    routinesTotal:(state.routines||[]).length,
    focusDone:(state.focusBlocks||[]).filter(x=>x.done).length,
    focusTotal:(state.focusBlocks||[]).length,
    tasksCompleted:(state.todos||[]).filter(x=>x.status==='completed' && String(x.completed_at||'').startsWith(date)).length,
  }
}
...
function migrateDaily(persisted,current){
  ...
  const merged = {
    ...current,
    ...incoming,
    systemMeta:{...current.systemMeta,...(incoming.systemMeta||{})},
    dailyHistory:Array.isArray(incoming.dailyHistory)?incoming.dailyHistory:[],
    activityLog:Array.isArray(incoming.activityLog)?incoming.activityLog:[],
  }
  const today = todayKey()
  const previous = incoming.systemMeta?.lastDailyReset
  if(!previous){
    merged.systemMeta.lastDailyReset=today
    return merged
  }
  if(previous!==today){
    const history=[dailySnapshot(merged,previous),...(merged.dailyHistory||[]).filter(x=>x.date!==previous)].slice(0,90)
    merged.dailyHistory=history
    merged.routines=(merged.routines||[]).map(x=>({...x,done:false,completed_at:null}))
    merged.focusBlocks=(merged.focusBlocks||[]).map(x=>({...x,done:false,completed_at:null}))
    merged.systemMeta.lastDailyReset=today
    merged.activityLog=cap([activity('system','New day started automatically',{from:previous,to:today}),...(merged.activityLog||[])])
  }
  return merged
}
...
export const useCommandStore = create(persist((set,get)=>({
  ...
  ensureDailyState:()=>set(s=>{
    const today=todayKey()
    const previous=s.systemMeta?.lastDailyReset
    if(!previous) return {systemMeta:{...s.systemMeta,lastDailyReset:today}}
    if(previous===today) return {}
    return {
      dailyHistory:[dailySnapshot(s,previous),...(s.dailyHistory||[]).filter(x=>x.date!==previous)].slice(0,90),
      routines:s.routines.map(x=>({...x,done:false,completed_at:null})),
      focusBlocks:s.focusBlocks.map(x=>({...x,done:false,completed_at:null})),
      systemMeta:{...s.systemMeta,lastDailyReset:today},
      activityLog:cap([activity('system','New day started automatically',{from:previous,to:today}),...s.activityLog]),
    }
  }),

  toggleFocus:id=>set(s=>{
    const current=s.focusBlocks.find(x=>x.id===id)
    const done=!current?.done
    return {
      focusBlocks:s.focusBlocks.map(x=>x.id===id?{...x,done,completed_at:done?nowIso():null}:x),
      activityLog:cap([activity('focus',`${done?'Completed':'Reopened'} focus: ${current?.block||'Focus'}`),...s.activityLog]),
    }
  }),
  updateFocus:(id,patch)=>set(s=>({focusBlocks:s.focusBlocks.map(x=>x.id===id?{...x,...patch}:x)})),

  toggleRoutine:id=>set(s=>{
    const current=s.routines.find(x=>x.id===id)
    const done=!current?.done
    return {
      routines:s.routines.map(x=>x.id===id?{...x,done,completed_at:done?nowIso():null}:x),
      activityLog:cap([activity('routine',`${done?'Completed':'Reopened'} routine: ${current?.name||'Routine'}`),...s.activityLog]),
    }
  }),
  resetRoutines:()=>set(s=>({
    routines:s.routines.map(x=>({...x,done:false,completed_at:null})),
    systemMeta:{...s.systemMeta,lastDailyReset:todayKey()},
    activityLog:cap([activity('routine','Daily routines reset manually'),...s.activityLog]),
  })),

  toggleTodo:id=>set(s=>{
    const current=s.todos.find(x=>x.id===id)
    const completed=current?.status!=='completed'
    return {
      todos:s.todos.map(x=>x.id===id?{...x,status:completed?'completed':'pending',completed_at:completed?nowIso():null}:x),
      activityLog:cap([activity('task',`${completed?'Completed':'Reopened'} task: ${current?.task||'Task'}`),...s.activityLog]),
    }
  }),
  addTodo:(task,priority='medium')=>set(s=>({
    todos:[{id:uid(),task,priority,status:'pending',created_at:nowIso(),completed_at:null},...s.todos],
    activityLog:cap([activity('task',`Added task: ${task}`),...s.activityLog]),
  })),
  updateTodo:(id,patch)=>set(s=>({todos:s.todos.map(x=>x.id===id?{...x,...patch}:x)})),
  deleteTodo:id=>set(s=>({todos:s.todos.filter(x=>x.id!==id)})),

  addEvent:event=>set(s=>({events:[...s.events,{id:uid(),color:'#2563EB',...event}]})),
  ...
  addProduct:product=>set(s=>({products:[...s.products,{id:uid(),...product}]})),
  ...
  addLead:lead=>set(s=>({
    leads:[{id:uid(),status:'new',created_at:nowIso(),...lead},...s.leads],
    activityLog:cap([activity('lead',`Added lead: ${lead.name||lead.business||'New lead'}`),...s.activityLog]),
  })),
  updateLead:(id,patch)=>set(s=>({leads:s.leads.map(x=>x.id===id?{...x,...patch}:x)})),
  deleteLead:id=>set(s=>({leads:s.leads.filter(x=>x.id!==id)})),
  moveLead:(id,status)=>set(s=>{
    const lead=s.leads.find(x=>x.id===id)
    return {
      leads:s.leads.map(x=>x.id===id?{...x,status,updated_at:nowIso()}:x),
      activityLog:cap([activity('lead',`${lead?.name||'Lead'} moved to ${status}`),...s.activityLog]),
    }
  }),

  addClient:client=>set(s=>({clients:[{id:uid(),...client},...s.clients]})),
  ...
  addResearch:item=>set(s=>({research:[{id:uid(),date:todayKey(),status:'pending',...item},...s.research]})),
  ...
  addIncome:item=>set(s=>({income:[{id:uid(),currency:'GBP',date:todayKey(),...item},...s.income]})),
  addExpense:item=>set(s=>({expenses:[{id:uid(),currency:'GBP',date:todayKey(),...item},...s.expenses]})),
  deleteIncome:id=>set(s=>({income:s.income.filter(x=>x.id!==id)})),
  deleteExpense:id=>set(s=>({expenses:s.expenses.filter(x=>x.id!==id)})),
  setMonthlyGoal:value=>set({monthlyGoal:Number(value)||0}),

  addMemory:memory=>set(s=>({
    memories:[{id:uid(),date:todayKey(),...memory},...s.memories],
    activityLog:cap([activity('memory',`Saved memory: ${memory.title||'Untitled'}`),...s.activityLog]),
  })),
  deleteMemory:id=>set(s=>({memories:s.memories.filter(x=>x.id!==id)})),

  addTeamMember:member=>set(s=>({team:[...s.team,{id:uid(),active:true,...member}]})),
  ...
  replaceState:data=>set(s=>({
    ...defaults,
    ...data,
    systemMeta:{...defaults.systemMeta,...(data?.systemMeta||{}),lastDailyReset:todayKey()},
    activityLog:cap([activity('system','Backup imported'),...(data?.activityLog||s.activityLog||[])]),
  })),
  exportState:()=>get(),
}),{
